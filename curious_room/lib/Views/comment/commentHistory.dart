import 'package:curious_room/Models/CommentHistoryModel.dart';
...
class CommentHistoryPage extends StatefulWidget {
  final int commentid;
  const CommentHistoryPage({Key? key, required this.commentid}) : super(key: key);
...
class _CommentHistoryPageState extends State<CommentHistoryPage> {
  Future<List<CommentHistoryModel>> history = Future.value([]);
  late List<CommentHistoryModel> value = [];
  @override
  void initState() {
    super.initState();
    history = getCommentHistory(widget.commentid);
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        children: [
          ...
          Expanded(
            child: FutureBuilder<List<CommentHistoryModel>>(
              future: history,
